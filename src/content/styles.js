...
.icon-btn.ghost:hover {
    background: rgba(65, 54, 241, 0.12);
    color: var(--primary);
    box-shadow: none;
}
.icon-btn:hover {
...
.error-state .error-advice {
    font-size: 12px;
    color: var(--text-sub);
    background: rgba(65, 54, 241, 0.08);
    border: 1px solid var(--glass-border);
    border-radius: 8px;
    padding: 8px 10px;
}
...
.xiaoet-toast.visible {
    opacity: 1;
    transform: translateY(0);
}

.xiaoet-toast[data-level="success"] { background: rgba(22, 163, 74, 0.94); }
.xiaoet-toast[data-level="warning"] { background: rgba(217, 119, 6, 0.94); }
.xiaoet-toast[data-level="error"] { background: rgba(220, 38, 38, 0.94); }
.error-state .error-action:hover {
...
/* Manual dark mode toggle (synced with settings page) */
:host(.dark-mode) {
    --bg-glass: var(--bg-glass-dark);
    --bg-card: var(--bg-card-dark);
    --text-main: #f1f5f9;
